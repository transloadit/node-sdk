...
- Default `maxBase64Bytes`: 10_000_000 (decoded bytes).
- Reject oversize inputs with a structured error and a hint to use URL import or tus uploads.
- Express JSON body limit should be set to allow slightly more than the base64 limit to account for
  encoding overhead (e.g., 15 MB).
...
- If `assembly_url` is provided, the tool attempts to resume uploads to that Assembly instead of
  creating a new one.
