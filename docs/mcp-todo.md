...
- Add upload resume support:
  - New tus upload storage abstraction (`TusUploadStore`) and a Node implementation
    (file-based by default).
  - Expose `resumeUploads()` that reuses tus upload URLs.
...
- Resume flow: interrupt upload, resume with same Assembly.
