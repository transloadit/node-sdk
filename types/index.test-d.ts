...
expectType<Promise<Assembly>>(
  transloadit.awaitAssemblyCompletion('1', {
    onAssemblyProgress: (assembly) => {
      expectType<Assembly>(assembly)
    },
    timeout: 1,
...
expectType<Promise<{ ok: string; success: boolean }>>(
  transloadit.replayAssemblyNotification('1', { param1: { a: 1 } })
)
expectType<Promise<{ count: number; items: ListedAssembly[] }>>(
  transloadit.listAssemblies({ param1: { a: 1 } })
)
...
expectType<Promise<{ count: number; items: ListedTemplate[] }>>(
  transloadit.listTemplates({ param1: { a: 1 } })
)
