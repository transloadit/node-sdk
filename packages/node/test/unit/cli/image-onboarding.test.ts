...
test('logout derives revocation credentials from the same snapshot as device-key consent', async () => {
  onTestFinished(() => {
    vi.mocked(resolveCliConfig).mockReset()
  })
  await writeFile(
    'credentials',
    'TRANSLOADIT_KEY=device-key\nTRANSLOADIT_SECRET=device-secret\nTRANSLOADIT_LOGIN_METHOD=device\n',
  )
  vi.mocked(resolveCliConfig).mockReturnValue({
    credentials: { authKey: 'replacement-app-key', authSecret: 'replacement-app-secret' },
  })
  const expected = new Transloadit({
    authKey: 'device-key',
    authSecret: 'device-secret',
  }).calcSignature({ auth: { expires: '2030-01-01T00:00:00Z' }, steps: {} }).signature
  let actual = ''
  vi.spyOn(Transloadit.prototype, 'revokeOwnAuthKey').mockImplementation(function (
    this: Transloadit,
  ) {
    actual = this.calcSignature({ auth: { expires: '2030-01-01T00:00:00Z' }, steps: {} }).signature
...
describe('image init', () => {
  ...
  test('a failed scaffold preserves a catalog updated by another Storage writer', async () => {
    await mkdir('app')
    const original = await vi.importActual<typeof import('node:fs/promises')>('node:fs/promises')
    onTestFinished(() => {
      vi.mocked(open).mockReset()
    })
    vi.mocked(open).mockImplementation(async (path, ...options) => {
      if (path === 'app/storage-image-example/page.tsx') {
        const catalog = JSON.parse(await readFile('transloadit.images.json', 'utf8'))
        catalog.images['website/concurrent.jpg'] = {
          path: 'website/concurrent.jpg',
          width: 800,
          height: 600,
        }
        await writeFile('transloadit.images.json', JSON.stringify(catalog))
        throw new Error('Cannot write example page')
      }
      return original.open(path, ...options)
    })
    await main(['image', 'init', 'website/', '--public'])
    expect(process.exitCode).toBe(1)
    expect(JSON.parse(await readFile('transloadit.images.json', 'utf8')).images).toHaveProperty(
      'website/concurrent.jpg',
    )
    expect(await readFile('transloadit-images.d.ts', 'utf8')).toContain(
      'Generated by transloadit storage',
    )
  })
