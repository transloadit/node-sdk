  ...
  _createClass(TransloaditClient, [{
      ...
      var sendRequest = function sendRequest() {
        _this._remoteJson(requestOpts, function (err) {
          ...
          if (!opts.isResumable) {
            ...
            return _this.awaitAssemblyCompletion(result.assembly_id, cb, progressCb);
          }

          var tusOpts = _extends({ waitForCompletion: opts.waitForCompletion }, result);
          _this._sendTusRequest(tusOpts, cb, progressCb);
        });
      };
    ...
    value: function __remoteJson(opts, cb) {
      ...
      if (method === 'post' || method === 'put' || method === 'del') {
        var extraData = _extends({ tus_num_expected_upload_files: opts.tus_num_expected_upload_files }, opts.fields);
        this._appendForm(req, opts.params, extraData);
      }
    }
  }, {
    key: '_sendTusRequest',
    value: function _sendTusRequest(opts, cb, _onProgress) {
      var uploadsDone = 0;
      var streamLabels = Object.keys(this._tus_streams);
      var tlClient = this;
      var totalBytes = 0;
      var uploadedBytes = 0;
      _onProgress = _onProgress || function () {};
      ...
      try {
        for (var _iterator3 = streamLabels[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          ...
          var file = this._tus_streams[label];
          var uploadSize = fs.statSync(file.path).size; // todo this will fail for streams without path
          ...
          var tusUpload = new tus.Upload(file, {
            ...
            onProgress: function onProgress(bytesUploaded) {
              ...
              _onProgress({
                uploadProgress: { uploadedBytes: uploadedBytes, totalBytes: totalBytes }
              });
            },
            onSuccess: function onSuccess() {
              ...
              if (uploadsDone === streamLabels.length) {
                ...
                if (opts.waitForCompletion) {
                  tlClient.awaitAssemblyCompletion(opts.assembly_id, cb, _onProgress);
                }
              }
            }
          });
