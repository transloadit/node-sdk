...
export const parseSafeTemplate = (
  ...
  const { silent = process.env.NODE_ENV === 'production' } = opts ?? {}
  let parsed
  let indent = '  '
  try {
    ...
    if (!silent) {
      console.error('templateContent', templateContent)
    }
    return [
      new StepParsingError(
        `Error parsing valid type from Template. ${error}. Input: ${templateContent}`,
        [],
        '',
      ),
      null,
  ...
  if (!success) {
    if (!silent) {
      console.error('zodIssuesWithContext', zodIssuesWithContext)
    }
    return [
      new StepParsingError(
...
export function getFirstStepNameThatDoesNotNeedInput(
  ...
  return (
    Object.keys(parsed.steps).find(
      (stepName) =>
        !botNeedsInput(parsed.steps[stepName].robot, stepName, parsed.steps[stepName]) &&
        !excludeBots.includes(parsed.steps[stepName].robot),
    ) ?? ''
  )
}

export const hasRobot = (templateContent: string, rName: string | RegExp, silent = false) => {
  const [templateError, template] = parseSafeTemplate(templateContent, { silent })
  if (templateError) {
    return false
  }
...
export function getRecommendations(templateContent: string, silent = false): Recommendation[] {
  const [templateError, template] = parseSafeTemplate(templateContent, { silent })
  if (templateError) {
    return []
  }
  ...
  const canJustRun = canAssemblyJustRun(templateContent)
  const hasFileServe = hasRobot(templateContent, '/file/serve')
  const hasStorageRobot = hasRobot(templateContent, /\/store$/)
  const hasImageResizeStep = hasRobot(templateContent, '/image/resize')
  const hasVideoThumbStep = hasRobot(templateContent, '/video/thumb')
  ...
  if (stepCount === 2 && canJustRun && hasFileServe) {
    ...
    hasRobot(templateContent, '/image/resize') &&
    !hasRobot(templateContent, '/image/optimize')
  ) {
    recommendations.push({
...
export function addImportRobot(templateContent: string): string {
  ...
  const hasImportRobot =
    !!getFirstStepNameThatDoesNotNeedInput(templateContent) &&
...
export function addFieldsInput(templateContent: string): string {
  ...
  for (const [, step] of entries(parsed.steps)) {
...
function getMostCommonExampleValue(
  ...
  exampleValueCounts.forEach((count, value) => {
    if (count > maxCount) {
      maxCount = count
    ...
    for (const [paramName, paramValue] of entries(step)) {
      if (typeof paramValue === 'string') {
        ...
        allFields.forEach((field) => {
          field.occurrences.forEach((occurrence) => {
...
export function validateInterpolatedTemplate(
  ...
  for (const [stepName, step] of entries(template.steps)) {
    ...
    if (!zodRes.success) {
      zodRes.error.errors.forEach((err) => {
